...
def get_als_pred(users, nodes, user_to_pred, alpha, factors, iterations, regularization):
...
def train(df_train: pl.DataFrame, df_eval: pl.DataFrame, model_name: str, alpha: float, factors: int, iterations: int, regularization: float):
    ...
    if model_name == "baseline":
        df_pred = get_als_pred(users, nodes, eval_users, alpha, factors, iterations, regularization)
...
def parse_args():
    ...
    if model_name != "baseline":
        raise ValueError("Other models are not implemented")

    return experiment_name, run_name, model_name, alpha, factors, iterations, regularization


def main():

    experiment_name, run_name, model_name, alpha, factors, iterations, regularization = parse_args()
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        df_pred = train(df_train, df_eval, model_name, alpha, factors, iterations, regularization)
